 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_tb_leeloolxpvimeo_nav_nav_renderer extends plugin_renderer_base {
    ...
    public function render_nav($template) {
        if (isset($template->config->show_current_section) && ($template->config->show_current_section == 1)) {
            return $this->render_from_template(
                'block_tb_leeloolxpvimeo_nav/coursenav_onesection',
                $template
            );
        } else {
            return $this->render_from_template(
                'block_tb_leeloolxpvimeo_nav/coursenav',
                $template
            );
        }
    }
